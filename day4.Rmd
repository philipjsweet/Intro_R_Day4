...
1. Examine the impact of the peptide on cell growth rates 
...
You collected this data before you knew of the wonders of Tidy data. The first step will be to tidy up the data. When you're complete the dataframe should have 45 rows and new column named 'Treatment'. *Bonus* Remove the 'CFU_' from in front of the peptides. This can be done using mutate() and 'str_replace()'
...
2. Now try using 'geom_jitter' instead of geom_point and note the difference
...
Now that we have tidy data, we can use tidyverse functions to calculate the impact of each fungal protein on the number of CFU. To do so, you will need to find the mean number of CFU for the negative control (PBS) for each biological replicate and then divide each row by it's respective mean. 
...
If done properly, the mean CFU's for the PBS treated A BioRep will be 320.
...
3. Make a summary table of the mean and standard deviation of ratio_CFU for each Treatment. If you calculated the ratio correect the mean of Fo321	will be 0.9875

4. Now make the same styple of overlayed 'geom_boxplot'/'geom_jitter' as we made above, but using the ratio CFU data. When using 'geom_boxplots', outlier's are represented using black points. Modify the shape of the 'geom_point()' so that you can clearly distinguish the individual replicates from the boxplot outlier's.
...
How does the plot for the ratio compare to the plot of the raw counts made in section 1.3? 
...
The plate reader you are using outputs a wide dataset ('Growth_data.csv') where each column is a seperate well and each row is a timepoint. You also have made key ('Platemap.csv') that contains that descriptive data that goes with each well. Start by reading in this data and looking at the structure. 
...
To tidy this data, you will need to conver the wide 'Growth Data' into a long format and then join on descritor the data stored in the 'Key' *Hint 1* when pivoting, you can use ':' to select all the columns in between (i.e colA:colX) 

If the data has been correctly tidied, the resulting dataframe should have 4080 rows.
...
1. Now plot growth curve by [Fo114] using 'geom_smooth', using a differnet color for each Concentration and 'facet_wrap()' to split out the biological replicates. Manually set the colors reflect the increase in concentration.  *Bonus* Transform the x-axis to be in hours instead of minutes
